...
func TestParseEthermintChainId(t *testing.T) {
	testCases := []struct {
		name     string
		chainID  string
		expError bool
	...
	for _, tc := range testCases {
		chainIDEpoch, err := ParseEthermintChainId(tc.chainID)
		if tc.expError {
			require.Error(t, err, tc.name)
			require.Nil(t, chainIDEpoch)

			require.False(t, IsValidEthermintChainID(tc.chainID), tc.name)
		} else {
			require.NoError(t, err, tc.name)
			require.Equal(t, tc.expInt, chainIDEpoch, tc.name)
			require.True(t, IsValidEthermintChainID(tc.chainID))
		}
	}
}
